...
# bash batchprnt.sh 30 8232688 8232689 8232690
# https://labelary.com/viewer.html design

PRINTER="172.30.191.54"
...
ora_pn["8236715"]="672042984282 CBL-MCIO-1445AM5B1-OC018"
...
function separator(){
	curl -s 'http://'${PRINTER}':9100/' -m 3 \
	  -H 'Accept: */*' \
	  -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/147.0.0.0 Safari/537.36' \
	  --data-raw $'^XA\n^PW380\n^LL200\n^CF0,19\n^FO25,25^A0N,50,25^FD'${1}'^FS^FO25,95^A0N,50,25^FD'${2}'^FS^FO25,155^A0N,50,25^FD'${3}'^FS^XZ'
}
...
for pn in "${argpn[@]}"; do
	...
	tta=(${ora_pn["${pn}"]})
	if [[ ${#pn} -ne 7 ]] || [[ ${#tta[@]} -eq 0 ]] ; then
		echo -e "  ${YLW}wrong $pn${NCL}"
		continue
	fi 
